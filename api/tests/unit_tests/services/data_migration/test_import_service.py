...
def test_mcp_tool_import_restores_exported_tool_list(monkeypatch):
    provider = type("Provider", (), {"id": "target-provider-id", "tools": "[]", "authed": False, "identity_mode": "off"})()
    report_items = []
...
def test_mcp_tool_existing_provider_records_id_mapping(monkeypatch, conflict_strategy):
    provider = type("Provider", (), {"id": "target-mcp-provider-id", "tools": "[]", "authed": False, "identity_mode": "off"})()
    id_mapping = {}
    id_mapping_details = []
